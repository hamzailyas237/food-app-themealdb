...
const MealSingle = ({ meal, heading, generateRandomMealHandler }) => {
  ...
  const isFavorite = favorites.some(favorite => favorite.idMeal === meal.id);
    ...
    <div className='section-wrapper'>
      <div className='container'>
        ...
        <div className='flex-row'>
          <div className='sc-title'>{heading ? heading : 'Meal Details'}</div>
          {heading && <button className='random-meal-button' onClick={generateRandomMealHandler}>{heading && 'Generate Another'}</button>}
        </div>
        <section className='sc-details bg-white'>
          <div className='details-head grid'>
            ...
            <div className='details-intro'>
              {
                heading &&
                <div onClick={isFavorite ? () => handleRemoveFromFavorites(meal.id) : () => handleAddToFavorites(meal)} style={{ float: 'right' }}><FaHeart color={isFavorite ? 'red' : 'gray'} /> </div>
              }
