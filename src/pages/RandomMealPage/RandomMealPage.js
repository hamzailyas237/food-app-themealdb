...
import "./RandomMealPage.scss";
import CategoryList from '../../components/Category/CategoryList';
import MealSingle from "../../components/Meal/MealSingle";
import { useMealContext } from '../../context/mealContext';
import { startFetchRandomMeal } from '../../actions/mealsActions';
...
const RandomMealPage = () => {
    ...
    const [generateRandomMeal, setGenerateRandomMeal] = useState(false)

    useEffect(() => {
        startFetchRandomMeal(dispatch);
    }, [generateRandomMeal]);

    const generateRandomMealHandler = () => {
        setGenerateRandomMeal(!generateRandomMeal)
    }
    ...
    return (
        <main className='main-content bg-whitesmoke'>
            {(mealLoading) ? <Loader /> : <MealSingle meal={singleMeal} heading={'Random Meal'} generateRandomMealHandler={generateRandomMealHandler} />}
